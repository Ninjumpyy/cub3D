...
/*   Updated: 2024/10/28 11:41:43 by tle-moel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	determine_y_offset(t_data *data, int *y_end)
{
	...
	{
		if (((data->player.y * PIXELS_PER_CELL) + (MINIMAP_HEIGHT / 2))
			> (data->map_height * PIXELS_PER_CELL))
			*y_end = data->map_height * PIXELS_PER_CELL;
		else if (((data->player.y * PIXELS_PER_CELL) - (MINIMAP_HEIGHT / 2))
			< 0)
...
void	determine_x_offset(t_data *data, int *x_end)
{
	...
	{
		if (((data->player.x * PIXELS_PER_CELL) + (MINIMAP_WIDTH / 2))
			> (data->map_width * PIXELS_PER_CELL))
			*x_end = data->map_width * PIXELS_PER_CELL;
		else if (((data->player.x * PIXELS_PER_CELL) - (MINIMAP_WIDTH / 2))
			< 0)
			*x_end = MINIMAP_WIDTH;
		else
...
void	draw_minimap_on_cub(t_data *data)
{
	...
	while (y < y_end)
	{
		...
		while (x < x_end)
		{
			draw_pixel(&(data)->minimap, (x - data->player.offset_x),
				(y - data->player.offset_y), find_cell_color(x, y, data));
			x++;
		}
		y++;
	}
...
int	find_cell_color(int x, int y, t_data *data)
{
	...
	else if (data->env.map[map_y][map_x] == ' ' ||
		data->env.map[map_y][map_x] == '\0' ||
		data->env.map[map_y][map_x] == '\n')
		return (convert_color(122, 242, 242, 242));
	else
